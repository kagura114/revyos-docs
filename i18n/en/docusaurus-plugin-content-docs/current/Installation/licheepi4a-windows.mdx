...
LicheePi4A currently supports two boot methods: [Booting from SD card](#sd-card) and [Booting from eMMC](#booting-from-emmc). This tutorial provides instructions for both methods. Please click to jump to the section that matches your desired flashing method.
